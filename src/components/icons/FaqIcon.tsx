...
type FaqIconProps = OnClick & {
  small?: boolean
}

export default function ({ onClick, small }: FaqIconProps) {
  const size = small ? 'p-1 h-6 w-6' : 'p-2 h-11 w-11'

  return (
    <div
      className={
        'flex items-center justify-center rounded-3xl cursor-pointer bg-opacity-30 bg-primary hover:animate-pulse active:scale-105 transition-transform ' +
        size
      }
      onClick={onClick}
    >
